// ==========================================
// 1. STYLE INJECTION (RESTORED ALL UI)
// ==========================================
const styleSheet = document.createElement("style");
styleSheet.innerText = `
    .btn-deposit { background: linear-gradient(135deg, #10b981, #059669) !important; color: white !important; font-weight: 800 !important; border: none !important; box-shadow: 0 4px 15px rgba(16, 185, 129, 0.4) !important; text-transform: uppercase; letter-spacing: 1px; transition: transform 0.2s ease; cursor: pointer; border-radius: 12px; }
    .btn-withdraw { background: linear-gradient(135deg, #ef4444, #b91c1c) !important; color: white !important; font-weight: 800 !important; border: none !important; box-shadow: 0 4px 15px rgba(239, 68, 68, 0.4) !important; text-transform: uppercase; letter-spacing: 1px; transition: transform 0.2s ease; cursor: pointer; border-radius: 12px; }
    .product-grid-wc { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; padding: 10px 0; }
    .product-card-wc { background: #fff; border-radius: 20px; overflow: hidden; box-shadow: 0 15px 30px rgba(0,0,0,0.08); border: 1px solid #f0f0f0; display: flex; flex-direction: column; }
...
// ==========================================
// RENDER HOME (ACTIVITIES RESTORED)
// ==========================================
const renderHomeScreen = async () => {
    appContent.innerHTML = '<p style="text-align: center; padding: 50px;">Syncing...</p>';
    try {
        ...
        const txns = histData.transactions || [];
        
        const activityList = txns.slice(0, 4).map(t => `
            <div class="activity-item">
                <span class="activity-type">${t.type.replace(/_/g, ' ')}</span>
                <span style="color:${t.amount > 0 ? '#10b981' : '#ef4444'}; font-weight:bold;">₦${Number(Math.abs(t.amount)).toLocaleString()}</span>
            </div>`).join('');

        appContent.innerHTML = `
            <div class="top-header" style="padding: 20px;"><h4>Hello, ${data.balance.full_name.split(' ')[0]}</h4></div>
            <div class="balance-card" style="background: #6a0dad; color: white; padding: 25px; border-radius: 20px; margin: 0 15px;">
                <small>Available Balance</small>
                <h1>₦ ${Number(data.balance.balance || 0).toLocaleString()}</h1>
            ...
            <div style="padding: 20px;">
                <div class="quick-actions" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px;">
                    <a href="#certificate" style="text-align:center; text-decoration:none; color:#333;"><div style="background:#f3e5f5; padding:12px; border-radius:12px;"><i class="fas fa-file-contract"></i></div><span style="font-size:10px;">Cert.</span></a>
                ...
                <div style="margin-top: 25px; background: white; border-radius: 15px; padding: 15px; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                    <h3 style="font-size: 15px; margin-bottom: 15px;">Recent Activities</h3>
                    ${activityList || '<p style="font-size:12px; color:#999;">No activities yet.</p>'}
                </div>
            </div>`;
    } catch (e) { logoutUser(); }
};
...
const renderTeamPage = async () => {
    ...
    appContent.innerHTML = `<div class="page-container" style="padding:20px;"><h2>My Team</h2><div style="background:#6a0dad; color:white; padding:20px; border-radius:15px; text-align:center; margin-bottom:20px;"><small>Team Bonus</small><h2>₦${Number(data.total_commission || 0).toLocaleString()}</h2></div>${list || '<p>No team yet.</p>'}</div>`;
};
...
const renderCertificatePage = () => {
    appContent.innerHTML = `<div class="page-container" style="padding:20px; text-align:center;"><h2>Legal Certificate</h2><img src="image.png" style="width:100%; border-radius:10px; box-shadow:0 5px 15px rgba(0,0,0,0.1);" onerror="this.src='https://placehold.co/600x800?text=Certificate+Image+Missing'"></div>`;
};
...
const renderRewardsPage = async () => {
    ...
    appContent.innerHTML = `<div class="page-container" style="padding:20px;"><h2>Rewards</h2><div style="background:#10b981; color:white; padding:20px; border-radius:15px; text-align:center; margin-bottom:20px;"><small>Total ROI</small><h2>₦${Number(data.total_daily_income || 0).toLocaleString()}</h2></div>${items || '<p>No rewards yet.</p>'}</div>`;
};

// OTHER CORE FUNCTIONS (PRODUCTS, VIP, WITHDRAW, DEPOSIT) - KEPT FULL LOGIC
const renderProductsPage = async () => {
    const res = await fetchWithAuth(`${API_BASE_URL}/users/allItems`);
    const data = await res.json();
    const items = data.items.map(i => `<div class="product-card-wc"><div class="product-image-wc"><img src="${i.itemimage}"></div><div class="product-info-wc"><h4>${i.itemname}</h4><div class="product-stats"><div><small>Price</small><br><strong>₦${Number(i.price).toLocaleString()}</strong></div><div><small>Daily</small><br><strong>₦${Number(i.dailyincome).toLocaleString()}</strong></div></div><button class="btn-invest-premium" data-plan-id="${i.id}" data-type="regular" style="margin-top:10px;">Invest Now</button></div></div>`).join('');
    appContent.innerHTML = `<div class="page-container" style="padding:20px;"><h2>Investments</h2><div class="product-grid-wc">${items}</div></div>`;
};
...
const renderVipPage = () => {
    ...
    appContent.innerHTML = `<div class="page-container" style="padding:20px;"><h2>VIP Plans</h2><div class="product-grid-wc">${items}</div></div>`;
};
...
const renderLoginScreen = () => {
    appContent.innerHTML = `<div class="auth-container" style="padding:40px 20px; text-align:center;"><h2>JJB24</h2><form id="loginForm" style="text-align:left;"><label>Email/Phone</label><input type="text" id="loginIdentifier" required style="width:100%; padding:12px; margin:8px 0; border-radius:10px; border:1px solid #ddd;" /><br><label>Password</label><input type="password" id="password" required style="width:100%; padding:12px; margin:8px 0; border-radius:10px; border:1px solid #ddd;" /><button type="submit" style="width:100%; background:#6a0dad; color:#fff; padding:15px; border:none; border-radius:10px; margin-top:15px;">Login</button></form><p style="margin-top:15px;">New? <a href="#register">Sign Up</a></p></div>`;
    document.getElementById('loginForm').addEventListener('submit', handleLogin);
};

const renderRegisterScreen = () => {
    appContent.innerHTML = `<div class="auth-container" style="padding:40px 20px;"><h2>Register</h2><form id="registerForm"><input type="text" id="fullName" placeholder="Name" required style="width:100%; padding:10px; margin:5px 0;" /><input type="email" id="email" placeholder="Email" required style="width:100%; padding:10px; margin:5px 0;" /><input type="tel" id="phone" placeholder="Phone" required style="width:100%; padding:10px; margin:5px 0;" /><input type="password" id="password" placeholder="Password" required style="width:100%; padding:10px; margin:5px 0;" /><input type="password" id="cpassword" placeholder="Confirm" required style="width:100%; padding:10px; margin:5px 0;" /><input type="text" id="referral" placeholder="Invite Code (Optional)" style="width:100%; padding:10px; margin:5px 0;" /><button type="submit" style="width:100%; background:#6a0dad; color:#fff; padding:15px; border:none; border-radius:10px;">Register</button></form></div>`;
...
// ==========================================
// FOMO POPUP (RESTORED TO CENTRAL POSITION)
// ==========================================
(function startSocialProof() {
    const names = ["Musa I.", "Chioma E.", "Tunde B.", "Ngozi O.", "Emeka A.", "Sani Y.", "Funke M."];
    const actions = ["just registered", "invested ₦50k", "joined VIP", "withdrew ₦25,000", "earned ₦5,000 bonus"];
