...
styleSheet.innerText = `
    ...
    .balance-card h2 { font-size: 38px !important; margin: 10px 0 25px; color: white !important; font-weight: 800 !important; }
    ...
    /* GRID FEATURES (FIXED PLANE VIEW) */
    ...
    .action-button span { font-size: 12px; font-weight: 800; color: var(--text-dark); }
    ...
    /* HISTORY/INPUTS */
    .history-item-text { color: var(--text-dark) !important; font-weight: 800 !important; }
    .form-group input, select { 
        width: 100%; padding: 18px; border-radius: 20px; border: 1.5px solid #E2E8F0; 
        background: #F8FAFC; box-sizing: border-box; font-weight: 700; color: var(--text-dark) !important;
    }
`;
...
announcementDiv.innerHTML = `
    <div class="modal-content">
        ...
        <h3 style="margin: 15px 0 10px; color: #111;">Official Community</h3>
        ...
        <a href="https://chat.whatsapp.com/Dw76vw5BJ68FONRPd6GPUi" target="_blank" onclick="closeWhatsappModal()" class="btn-whatsapp" style="background: #25D366; color: white !important; font-weight: 800; padding: 16px; border-radius: 15px; text-decoration: none; display: block; margin-top: 20px;">JOIN NOW</a>
        <button onclick="closeWhatsappModal()" style="background:none; border:none; color:#aaa; margin-top:20px; cursor:pointer; font-size:12px; font-weight:700;">Close</button>
    </div>
`;
...
const showSuccessModal = (message) => {
    ...
    if (modalMessage) modalMessage.innerHTML = `<i class="fas fa-check-circle" style="font-size: 50px; color: #10b981; margin-bottom: 20px; display:block;"></i><h3 style="color:#111;">Success</h3><p style="color:#64748B; font-weight:600;">${message}</p>`;
...
window.copyReferralLink = async (referralCode) => {
    ...
    try { await navigator.clipboard.writeText(fullLink); showSuccessModal('Referral link copied!'); } catch (err) { prompt("Copy link:", fullLink); }
};
...
const getReferralCardHTML = (code) => `
    <div style="background: white; border-radius: 28px; padding: 25px; margin-bottom: 25px; box-shadow: var(--card-shadow); border: 1.5px dashed var(--accent-primary); text-align:center;">
        <small style="color: var(--text-muted); font-weight: 800; font-size: 10px; text-transform: uppercase;">Invitation Code</small>
        <div style="font-weight: 800; font-size: 20px; color: var(--text-dark); margin: 10px 0 20px;">${code}</div>
...
const handleOTPVerification = async (e, email) => {
    ...
    if (res.ok) { showSuccessModal('Account Verified!'); renderLoginScreen(); } else { alert('Failed'); }
};
...
// ==========================================
// 5. RENDER FUNCTIONS (FIXED PLANE LOOKS)
// ==========================================
...
const renderHomeScreen = async () => {
    ...
    let activityHTML = "<p style='text-align:center; font-size:12px; color:#94A3B8; font-weight:800; padding:20px;'>NO RECENT LOGS</p>";
    try {
        ...
        if (hData.transactions.length > 0) {
            activityHTML = hData.transactions.slice(0, 4).map(t => `
                <div class="activity-item">
                    <div><div class="history-item-text" style="font-size:14px;">${t.type.replace(/_/g, ' ')}</div><small style="color:#94A3B8; font-weight:700;">${new Date(t.created_at).toLocaleDateString()}</small></div>
    ...
    appContent.innerHTML = `
        <div class="top-header">
            <div class="user-greeting"><h4>Hello, ${user.full_name.split(' ')[0]}</h4><p>Exclusive Member</p></div>
        ...
        <div class="activity-card">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;"><h3 style="margin:0; font-size:16px;">Recent Stream</h3><a href="#history" style="font-size:11px; font-weight:800; color:var(--accent-primary); text-decoration:none;">VIEW ALL</a></div>
...
const renderMePage = async () => {
    ...
    const pinText = user.has_pin ? "Update Transaction PIN" : "Setup Transaction PIN";
    const pinHash = user.has_pin ? "#reset-pin" : "#set-pin";
    ...
    appContent.innerHTML = `
        <div style="padding: 20px 20px 100px;">
            <div style="background: white; padding: 35px 20px; border-radius: 32px; text-align: center; box-shadow: var(--card-shadow); margin-bottom: 25px;">
                <div style="width: 80px; height: 80px; background: #F1F5F9; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px; font-size: 28px; color:var(--text-dark);"><i class="fas fa-user-tie"></i></div>
                <h3 style="margin: 0; color:var(--text-dark); font-weight:800;">${user.full_name}</h3>
                <p style="color: var(--text-muted); font-size: 14px; font-weight:700;">${user.phone_number}</p>
            ...
            <div style="background: white; border-radius: 32px; overflow: hidden; box-shadow: var(--card-shadow);">
                ...
                <a href="${pinHash}" style="display:flex; justify-content:space-between; padding:22px; border-bottom:1px solid #F1F5F9; text-decoration:none; color:var(--text-dark); font-weight:800;"><span>${pinText}</span><i class="fas fa-chevron-right" style="font-size:12px;color:#CBD5E1;"></i></a>
                <a href="#history" style="display:flex; justify-content:space-between; padding:22px; border-bottom:1px solid #F1F5F9; text-decoration:none; color:var(--text-dark); font-weight:800;"><span>Records</span><i class="fas fa-chevron-right" style="font-size:12px;color:#CBD5E1;"></i></a>
                <a href="javascript:void(0)" onclick="window.logoutUser()" style="display:block; padding:22px; color:#EF4444; font-weight:900; text-decoration:none;">Sign Out Portfolio</a>
            </div>
        </div>`;
};

// ==========================================
// 6. ROUTER & SYSTEM HANDSHAKE (RESTORED)
// ==========================================
...
(function startSocialProof() {
    ...
    style.innerHTML = `#fomo-popup { position: fixed; bottom: 85px; left: 50%; transform: translateX(-50%) translateY(200%); background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(12px); border: 1px solid rgba(255, 255, 255, 0.8); border-left: 5px solid #10B981; padding: 12px 18px; border-radius: 20px; box-shadow: 0 15px 35px -5px rgba(0, 0, 0, 0.1); z-index: 9999; transition: all 0.6s cubic-bezier(0.34, 1.56, 0.64, 1); display: flex; align-items: center; gap: 12px; width: 90%; max-width: 340px; pointer-events: none; } #fomo-popup.show { transform: translateX(-50%) translateY(0); } .fomo-content { display: flex; flex-direction: column; } .fomo-name { font-size: 13px; font-weight: 800; color: #0F172A; } .fomo-desc { font-size: 12px; color: #64748B; font-weight: 700; } .fomo-meta { font-size: 10px; color: #94A3B8; margin-top: 1px; }`;
    document.head.appendChild(style);

    const popup = document.createElement('div'); popup.id = 'fomo-popup'; popup.innerHTML = `<div id="fomo-icon" style="font-size:20px;">👋</div><div class="fomo-content"><span class="fomo-name" id="fomo-name">...</span><span class="fomo-desc" id="fomo-action">...</span><span class="fomo-meta"><span id="fomo-location">Lagos</span> • <span id="fomo-time">Just now</span></span></div>`;
    ...
    function trigger() {
        ...
        document.getElementById('fomo-time').innerText = fomoData.times[Math.floor(Math.random() * fomoData.times.length)];
