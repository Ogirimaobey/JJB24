...
    
    // MODIFIED: Check if we are sending a file (FormData)
...
// --- UPDATED: DEPOSIT PAGE (ONLY MANUAL RECEIPT SYSTEM ADDED) ---
...
                        <input type="number" id="receiptAmount" placeholder="Enter amount transferred" required style="width:100%; padding:12px; border:1px solid #ddd; border-radius:10px;">
                    </div>
                    <div class="form-group" style="margin-bottom:15px;">
                        <label>Upload Payment Proof (Screenshot)</label>
                        <input type="file" id="receiptFile" accept="image/*" required style="width:100%; padding:10px; border:1px dashed #6a0dad; border-radius:10px; background:#f3e8ff;">
                    </div>
                    <button type="submit" class="btn-deposit" style="width:100%; padding:15px; border-radius:12px; font-weight:800;">SUBMIT FOR APPROVAL</button>
                </form>
            </div>
        </div>`;
...
        appContent.innerHTML = '<div style="text-align:center; padding-top:100px;"><h4>Uploading Receipt...</h4><p>Please do not close this page.</p></div>';

        try {
            const response = await fetchWithAuth(`${API_BASE_URL}/transactions/upload-receipt`, {
                method: 'POST',
                body: formData // fetchWithAuth now correctly handles FormData boundaries
            });

            if (response && response.ok) {
                showSuccessModal('Receipt uploaded! Admin will verify within 24 hours.');
