// ==========================================
// 1. CONFIGURATION & STYLING INJECTION (WORLD-CLASS UI + DESKTOP FIT)
// ==========================================
...
styleSheet.innerText = `
    ...
    /* DESKTOP RESPONSIVENESS: CENTER THE APP FRAME FOR MACBOOK/WEB */
    body { 
        font-family: 'Plus Jakarta Sans', sans-serif !important; 
        background: #E2E8F0; /* Desktop Background */
        margin: 0; 
        display: flex;
    ...
    .product-stats { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 20px; }
    ...
    /* BOTTOM NAV (FIXED FOR DESKTOP FRAME) */
    .bottom-nav {
        position: fixed; bottom: 0; left: 50%; transform: translateX(-50%);
        width: 100%; max-width: 500px; height: var(--nav-height);
        background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(20px);
        display: flex; justify-content: space-around; align-items: center;
        border-top: 1px solid #E2E8F0; z-index: 10000; padding-bottom: 15px;
    }
...
const getReferralFromUrl = () => {
    ...
    if (fullHash.includes('?ref=')) return fullHash.split('?ref=')[1].split('&')[0];
    return '';
};
...
const getReferralCardHTML = (code) => {
    ...
    <div style="background: white; border-radius: 28px; padding: 25px; margin-bottom: 25px; box-shadow: var(--card-shadow); border: 1px dashed var(--accent-primary);">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 15px;">
            <div style="width: 40px; height: 40px; background: #F5F3FF; border-radius: 12px; display: flex; align-items: center; justify-content: center; color: var(--accent-primary);"><i class="fas fa-gift"></i></div>
            <div>
                <small style="color: #64748B; font-weight: 700; font-size: 10px; text-transform: uppercase;">Invitation Code</small>
                <div style="font-weight: 800; font-size: 18px; color: var(--text-dark);">${code}</div>
            </div>
        </div>
        <button onclick="window.copyReferralLink('${code}')" class="btn-deposit" style="width: 100%; padding: 14px; background: var(--accent-primary) !important;">COPY INVITE LINK</button>
    </div>`;
};
...
const handleRegister = async (event) => {
    ...
    if (!document.getElementById('termsCheckbox').checked) return alert('Agree to terms');
    try {
...
window.handleForgotPassword = async () => {
    ...
    if (!email) return;
    try {
        const response = await fetch(`${API_BASE_URL}/users/forgot-password`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email }) });
    ...
    appContent.innerHTML = `
        <div class="auth-container">
            ...
            <form id="loginForm">
                ...
                <div class="form-group"><label>Secure Password</label><input type="password" id="password" placeholder="••••••••" required /></div>
                <div style="text-align: right; margin-bottom: 30px;"><a href="javascript:void(0)" onclick="handleForgotPassword()" style="color: var(--accent-primary); font-size: 12px; font-weight: 700; text-decoration:none;">Reset Password</a></div>
                <button type="submit" class="btn-auth" style="width:100%; padding:18px; background: var(--accent-primary); color:white;">SECURE LOGIN</button>
            </form>
...
const renderRegisterScreen = () => {
    ...
    <div class="auth-container">
        <div class="auth-logo">JJB24</div>
        <h2 style="font-weight: 800;">Join Community</h2>
        <form id="registerForm">
            ...
            <div class="form-group"><label>Confirm</label><input type="password" id="cpassword" required /></div>
            <div class="form-group"><label>Referral Code</label><input type="text" id="referral" value="${ref}" ${ref ? 'readonly' : ''} /></div>
            <div style="display: flex; gap: 10px; margin: 25px 0; text-align: left;"><input type="checkbox" id="termsCheckbox" required /><label style="font-size: 11px;">I accept the Investment Terms & Policy.</label></div>
            <button type="submit" class="btn-auth" style="width:100%; padding:18px; background: var(--accent-primary); color:white;">CREATE ACCOUNT</button>
        </form>
    </div>`;
...
const renderHomeScreen = async () => {
    ...
    if (userEmail === 'audit@flutterwave.com') {
        complianceCardHTML = `<div style="background: white; border-radius: 28px; padding: 25px; margin: 0 20px 25px; box-shadow: var(--card-shadow); border-left: 6px solid #F59E0B;"><h3 style="margin: 0; font-size: 14px; font-weight: 800;">Compliance Audit</h3><p style="font-size: 13px; color: #475569; margin: 10px 0 0;">CAC Address: Monaya Rd, Ogoja 550101, Cross River.</p></div>`;
    }

    let activityHTML = "<p style='text-align:center; font-size:12px; color:#94A3B8; padding:20px;'>Clean History</p>";
    try {
        ...
        if (hData.transactions.length > 0) {
            activityHTML = hData.transactions.slice(0, 4).map(t => `
                <div class="activity-item">
                    <div><div style="font-weight:700; font-size:13px;">${t.type.replace(/_/g, ' ')}</div><small style="color:#94A3B8;">${new Date(t.created_at).toLocaleDateString()}</small></div>
    ...
    appContent.innerHTML = `
        <div class="top-header">
            <div class="user-greeting"><h4>Hi, ${user.full_name.split(' ')[0]}</h4><p>Exclusive Member</p></div>
        ...
        <div class="balance-card">
            ...
            <div style="display:flex; gap:12px;">
                <a href="#deposit" class="btn-deposit" style="flex:1; padding:15px; text-decoration:none;">FUND WALLET</a>
                <a href="#withdraw" class="btn-withdraw" style="flex:1; padding:15px; text-decoration:none;">WITHDRAW</a>
            </div>
        </div>
        ...
        <div class="activity-card">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;"><h3 style="margin:0; font-size:16px; font-weight: 800;">Stream</h3><a href="#history" style="font-size:11px; font-weight:700; color:var(--accent-primary); text-decoration:none;">VIEW ALL</a></div>
...
const renderWithdrawPage = async () => {
    ...
    appContent.innerHTML = `<div style="padding:20px 20px 100px;"><h2 style="font-weight:800; font-size:24px; margin-bottom:25px;">Liquidate</h2><div style="background:linear-gradient(135deg,#1E293B,#0F172A); padding:30px; border-radius:32px; color:white;"><div style="text-align:center; margin-bottom:30px; background:rgba(255,255,255,0.05); padding:20px; border-radius:20px;"><small style="opacity:0.5;font-weight:800;">AVAILABLE</small><h2 style="margin:5px 0 0 0; font-size:32px;">₦ ${Number(bal).toLocaleString()}</h2></div><form id="withdrawForm"><div class="form-group"><label style="color:white;opacity:0.6;">Amount (₦)</label><input type="number" id="amount" min="800" step="0.01" style="background:white; border:none; padding:18px; border-radius:18px; font-weight:800;" required /></div><div id="feeContainer" style="background:rgba(16,185,129,0.1); padding:15px; border-radius:15px; margin-bottom:15px; display:none;"><div style="display:flex; justify-content:space-between; font-size:12px;"><span>Fee (9%):</span><span id="feeDisplay" style="color:#EF4444;">-₦0</span></div><div style="display:flex; justify-content:space-between; font-weight:800; margin-top:10px;"><span>Final:</span><span id="finalDisplay" style="color:#10B981;">₦0</span></div></div><div class="form-group"><label style="color:white;opacity:0.6;">Bank</label><select id="bankName" required style="background:white;border:none;"><option value="OPay">OPay</option><option value="PalmPay">PalmPay</option><option value="Moniepoint">Moniepoint</option></select></div><div class="form-group"><label style="color:white;opacity:0.6;">Account Number</label><input type="text" id="accountNumber" style="background:white;border:none;" required /></div><div class="form-group"><label style="color:white;opacity:0.6;">Account Name</label><input type="text" id="accountName" style="background:white;border:none;" required /></div><div class="form-group"><label style="color:#F59E0B;font-weight:800;">Security PIN</label><input type="password" id="withdrawPin" maxlength="4" style="background:white; border:none; padding:18px; border-radius:18px; text-align:center; letter-spacing:10px; font-size:24px;" required /></div><button type="submit" class="btn-withdraw" style="width:100%; padding:20px; background:var(--accent-success)!important;">WITHDRAW FUNDS</button></form></div></div>`;
    
    const amtInp = document.getElementById('amount');
    const feeCont = document.getElementById('feeContainer');
    amtInp.addEventListener('input', () => {
        const v = parseFloat(amtInp.value);
        if(v >= 800) {
            const fee = (v * 0.09).toFixed(2);
            document.getElementById('feeDisplay').innerText = `-₦${Number(fee).toLocaleString()}`;
            document.getElementById('finalDisplay').innerText = `₦${Number(v-fee).toLocaleString()}`;
            feeCont.style.display = 'block';
        } else feeCont.style.display = 'none';
    });
    ...
    document.getElementById('withdrawForm').addEventListener('submit', async (e) => {
        ...
        const r = await res.json(); if(res.ok) showSuccessModal(r.message); else alert(r.message);
    });
};
...
const renderHistoryPage = async () => {
    ...
    const data = await res.json();
    const list = (data.transactions || []).map(t => `<div style="background:white; border-radius:20px; padding:20px; margin-bottom:12px; display:flex; justify-content:space-between; align-items:center; box-shadow:var(--card-shadow);"><div><div style="font-weight:700; text-transform:uppercase;">${t.type.replace(/_/g, ' ')}</div><small style="color:#94A3B8;">${new Date(t.created_at).toLocaleDateString()}</small></div><div style="color:${t.amount > 0 ? '#10B981' : '#0F172A'}; font-weight:800;">₦${Number(Math.abs(t.amount)).toLocaleString()}</div></div>`).join('');
...
const renderSupportPage = () => { 
    appContent.innerHTML = `<div style="padding:20px 20px 100px;"><h2 style="font-weight:800; font-size:24px;">Concierge</h2><div style="background:white; padding:35px; border-radius:32px; text-align:center; box-shadow:var(--card-shadow);"><div style="width:70px;height:70px;background:#DCFCE7;border-radius:24px;display:flex;align-items:center;justify-content:center;margin:0 auto 20px;"><i class="fab fa-whatsapp" style="font-size:35px;color:#25D366;"></i></div><h3 style="font-weight:800;">WhatsApp Support</h3><p style="color:#64748B;font-size:14px;margin-bottom:25px;">Fastest response for yield queries and verification assistance.</p><a href="https://chat.whatsapp.com/Dw76vw5BJ68FONRPd6GPUi" target="_blank" class="btn-deposit" style="background:#25D366!important;padding:18px;text-decoration:none;">OPEN CHAT</a></div></div>`; 
};

const renderCertificatePage = () => { 
    appContent.innerHTML = `<div style="padding:20px 20px 100px;"><h2 style="font-weight:800; font-size:24px;">Legal</h2><div style="background:white; padding:15px; border-radius:28px; box-shadow:var(--card-shadow);"><img src="image.png" style="width:100%; border-radius:20px;" onerror="this.parentElement.innerHTML='<p style=\'padding:40px;color:#94A3B8;text-align:center;\'>Loading Legal Docs...</p>'"></div></div>`; 
};

const renderChangePasswordPage = async () => {
    appContent.innerHTML = `<div style="padding: 20px;"><h2 style="font-weight:800; margin-bottom:25px;">Security Control</h2><div style="background:white; padding:30px; border-radius:28px; box-shadow:var(--card-shadow);"><form id="changePasswordForm"><div class="form-group"><label>Current Password</label><input type="password" id="oldPassword" required></div><div class="form-group"><label>New Security Key</label><input type="password" id="newPassword" required></div><button type="submit" class="btn-deposit" style="width:100%; padding:18px; background:var(--text-dark)!important;">UPDATE CREDENTIALS</button></form></div></div>`;
    document.getElementById('changePasswordForm').addEventListener('submit', async (e) => {
        ...
        const result = await res.json();
        if (res.ok) { showSuccessModal("Updated!"); setTimeout(() => { window.location.hash = '#me'; router(); }, 2000); } else alert(result.message);
    });
};

const renderSetPinPage = async () => {
    appContent.innerHTML = `<div style="padding:20px;"><h2 style="font-weight:800;">Security PIN</h2><div style="background:white; padding:40px; border-radius:28px; text-align:center; box-shadow:var(--card-shadow);"><form id="pinForm"><p style="color:#64748B;font-size:14px;margin-bottom:30px;">Set 4-digit PIN.</p><input type="password" id="pinInput" maxlength="4" style="text-align:center; font-size:32px; letter-spacing:15px; border:none; background:#F1F5F9; padding:20px; border-radius:18px;" required><button type="submit" class="btn-deposit" style="width:100%; margin-top:30px; padding:18px;">ACTIVATE PIN</button></form></div></div>`;
...
const renderResetPinPage = async () => {
    appContent.innerHTML = `<div style="padding:20px;"><h2 style="font-weight:800;">Transaction PIN</h2><div style="background:white; padding:40px; border-radius:28px; text-align:center; box-shadow:var(--card-shadow);"><form id="resetPinForm"><p style="color:#64748B;font-size:14px;margin-bottom:30px;">Define code.</p><input type="password" id="newPinInput" maxlength="4" style="text-align:center; font-size:32px; letter-spacing:15px; border:none; background:#F1F5F9; padding:20px; border-radius:18px;" required><button type="submit" class="btn-deposit" style="width:100%; margin-top:30px; padding:18px;">RESET PIN</button></form></div></div>`;
    document.getElementById('resetPinForm').addEventListener('submit', async (e) => {
        ...
        if (res.ok) { showSuccessModal("PIN Updated!"); setTimeout(() => { window.location.hash = '#me'; router(); }, 2000); } else alert("Failed.");
    });
};
...
// ==========================================
// 7. FULL FOMO ENGINE (1,000+ LINES DATA INTEGRITY)
// ==========================================
